...
- [ ] Thêm `Task<IReadOnlyList<ExamSubmission>> GetBySessionAsync(Guid sessionId, CancellationToken ct = default)` — `Where(x => x.SessionId == sessionId).OrderByDescending(x => x.Created)`.
- [ ] Thêm `Task<IReadOnlyList<ExamSubmission>> GetBySessionAndStudentAsync(Guid sessionId, Guid studentId, CancellationToken ct = default)` — thêm điều kiện `x.StudentId == studentId`.
- [ ] Verify: `dotnet build ExamHub.Core`.
...
- [ ] Thêm `GetBySessionAsync` và `GetBySessionAndStudentAsync` uỷ quyền xuống repo.

## Task 3: Backend — controller endpoints
**File:** `ExamHub.API/Controllers/Exam/ExamSubmissionController.cs`
- [ ] `GET by-session/{sessionId:guid}` `[Authorize(Roles="Admin,Teacher")]` → list (không kèm answers).
- [ ] `GET by-session/{sessionId:guid}/student/{studentId:guid}` → list các lần nộp của HS (không kèm answers).
- [ ] Verify build + kiểm tra `ExamHub.API.json` (Swagger) không lỗi.
...
- [ ] `submissionService.getBySession(sessionId)`, `getBySessionAndStudent(sessionId, studentId)`.
- [ ] `SUBMISSION_KEYS.bySession`, `bySessionStudent`; hooks `useSubmissionsBySessionQuery(sessionId)`, `useMySessionSubmissionsQuery(sessionId, studentId)`.

## Task 5: Frontend — tổng quát hoá `SubmissionsDrawer` theo session
**File:** `exam_hub_web/src/pages/exams/SubmissionsDrawer.tsx`
- [ ] Props đổi thành `{sessionId?: string; onClose}` (bỏ nhánh examId, hoặc nhận cả hai). Dùng `useSubmissionsBySessionQuery`.
- [ ] `SubmissionCard` tự nạp đề bằng `useExamWithQuestionsQuery(sub.examId)` (bỏ prop `questionContent` truyền từ ngoài); tự tính `questionContent`.
- [ ] Tiêu đề Drawer: "Bài nộp kỳ thi".

## Task 6: Frontend — bỏ nút "Bài nộp" ở `ExamListPage`
**File:** `exam_hub_web/src/pages/exams/ExamListPage.tsx`
- [ ] Gỡ state `submissionsExamId`, nút mở drawer, và `<SubmissionsDrawer/>` (đã chuyển sang kỳ thi).

## Task 7: Frontend — nút "Bài nộp" ở `ExamSessionListPage`
**File:** `exam_hub_web/src/pages/exams/ExamSessionListPage.tsx`
- [ ] Thêm action "Bài nộp" mỗi kỳ thi → set `submissionsSessionId` → render `<SubmissionsDrawer sessionId=…/>`.
...
- [ ] Thêm nút **"Xem kết quả"** vào chân thẻ khi `usedAttempts > 0` (cạnh nút hành động).
- [ ] `SessionResultsModal`: dùng `useMySessionSubmissionsQuery(sessionId, user.id)` liệt kê lần nộp (STT, điểm, trạng thái Tag, thời gian nộp); bấm dòng → `navigate('/student/exam/result?submissionId='+id)`.

## Task 9: Verify tổng thể + commit
- [ ] Build Core, tsc, eslint, vite build sạch (trừ lỗi pre-existing).
- [ ] Commit backend và frontend theo nhóm task.
- [ ] Cần **khởi động lại API** để nạp endpoint mới.
